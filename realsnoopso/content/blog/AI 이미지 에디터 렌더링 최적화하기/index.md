...
![[Screen Recording 2023-08-29 at 2.06.02 PM.gif]]
...
![[Screenshot 2023-08-29 at 12.25.24 PM.png]]
...
![[Screenshot 2023-09-13 at 2.25.08 PM.png]]
...
![[Screen Recording 2023-08-29 at 9.50.22 PM.mov]]

# 배운 점

1. 아쉽게도 기능을 런칭한 이후 사용자 만족도 변화를 추적해보았을 때 유의미하게 좋아지지 못했다. 여러가지 원인이 있었겠지만 우선 기술적인 한계가 컸다. 컨셉 바를 변경할 때마다
2. 유연한 조직에서 적응하려면 최대한 열어두고 짜야 하는 구나를 느꼈음. 중간에 사용자 피드백을 받아 슬라이더의 기준점이 뒤바뀐 상황이 있었는데 그 부분을 고려하지 않고 짜서 로직이 복잡해졌다.
3. zustand 와 같은 전역 상태 라이브러리를 사용할 때 최대한 상태를 조심스럽게 다뤄야 한다고 느꼈습니다. 자식의 상태를 변경했는데, 부모가 계속 재렌더링 되는 현상을 겪었음. 이를 통해 전역 상태 라이브러리를 사용할 때는 필요한 상태만 저장해야 한다는 것을 느꼈음.
4. 클릭하면 바로 변화되어야 하는 에디터라는 도메인을 다루면서 렌더링 횟수에 따라 사용자로서 불편함을 유발할 수 있다는 것을 체감하여 렌더링 최적화에 더 신경을 많이 써야겠구나 하고 느꼈음
